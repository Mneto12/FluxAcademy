...
class Restore_Database {
                            ...
                            if (preg_match('/;$/', $line)) {
                                ...
                                if(mysqli_query($this->conn, $sql)) {
                                    if (preg_match('/^CREATE TABLE `([^`]+)`/i', $sql, $tableName)) {
                                        $this->obfPrint("Tabla restaurada: `" . $tableName[1] . "`");
                                    }
                                    $sql = '';
                                } else {
                                    throw new Exception("ERROR: SQL error: " . mysqli_error($this->conn));
                                }
                            }
    ...
    protected function gunzipBackupFile() {
        ...
        $this->obfPrint('Archivo comprimido de respaldo: ' . $source . '... ', 1, 1);
